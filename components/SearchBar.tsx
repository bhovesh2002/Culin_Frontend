...
export default function SearchBar() {
    ...
    const handleSearch = async (e: any) => {
        ...
        try {
            const response = await fetch(`http://localhost:8080/find/product?product=${product}`, {
                method: 'GET',
                headers: {
            ...
            const countResult = Object.keys(data).length;
            setNumberOfResult(countResult);
            setSearchQuery(product);
    ...
    const searchButton = () => {
        return (
            <div className="grid place-items-center h-full w-12 text-gray-300">
                <button className="grid place-items-center h-12 w-12 text-gray-300" type="submit" disabled={isPending}>
